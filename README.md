...
Project Workflow
Hardware initialization and system check.
Fruit enters Stage 1 (Conveyor).
Sensors (Gas, pH, Distance) collect environmental and fruit data.
Camera captures images of the fruit.
Images are sent to Gemini AI for analysis.
The dashboard displays the AI confidence and defect results.
Fruit is routed based on the AI analysis (e.g., washing, sorting).
...
Confirm the correct Arduino COM port.
Confirm the correct camera index.
Confirm that the Gemini API key is valid.
Confirm that the camera feed is working.
Confirm that the Arduino is communicating correctly.
Start the SAFEFRUIT AI inspection workflow.
